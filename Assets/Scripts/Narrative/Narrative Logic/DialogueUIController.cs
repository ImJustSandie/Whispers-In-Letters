...
public class DialogueUIController : MonoBehaviour
{
    ...
    [Tooltip("Contenedor con HorizontalLayoutGroup para imágenes pequeñas (#small_image).")]
    [SerializeField] private RectTransform smallImageContainer;
    [Tooltip("Imagen grande para #big_image.")]
    [SerializeField] private Image bigImageDisplay;
    [Tooltip("Prefab con componente Image para instanciar en smallImageContainer.")]
    [SerializeField] private GameObject smallImagePrefab;
    ...
    private bool hasDisplayedImages = false;
    ...
    private void Awake()
    {
        ...
        if (tagProcessor != null)
        {
            ...
            tagProcessor.OnSmallImageRequested += HandleSmallImageRequested;
            tagProcessor.OnBigImageRequested += HandleBigImageRequested;
        }
        else
        {
    ...
    private void HandleSmallImageRequested(List<string> imageIds)
    {
        ClearSmallImages();
        ...
        NarrativeImageDatabase db = tagProcessor != null ? tagProcessor.ImageDatabase : null;

        foreach (string id in imageIds)
        {
            Sprite sprite = db != null ? db.GetImage(id) : null;
            ...
            GameObject imgObj = Instantiate(smallImagePrefab, smallImageContainer);
        ...
        smallImageContainer.gameObject.SetActive(true);
        hasDisplayedImages = true;
    }

    private void HandleBigImageRequested(string imageId)
    {
        ClearBigImage();
        ...
        NarrativeImageDatabase db = tagProcessor != null ? tagProcessor.ImageDatabase : null;
        Sprite sprite = db != null ? db.GetImage(imageId) : null;
    ...
    {
        if (smallImageContainer == null) return;

        foreach (Transform child in smallImageContainer)
        {
            Destroy(child.gameObject);
        }

        smallImageContainer.gameObject.SetActive(false);
    }

    private void ClearBigImage()
    {
        if (bigImageDisplay == null) return;

        bigImageDisplay.sprite = null;
    ...
    public void DisplayNextLine()
    {
        ...
        if (story.canContinue)
        {
            ...
            // Limpieza persistente de imágenes: solo se borran si keep_image NO está presente
            if (hasDisplayedImages && tagProcessor != null && !tagProcessor.HasKeepImageTag(story.currentTags))
            {
                ClearAllImages();
            }
